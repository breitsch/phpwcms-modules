...
if(isset($_POST['fb_name'])) {
  ...
  $plugin_fb['data']['values'] = array(
    ...
    'fb_tweet_tweetto'		=> empty($_POST['fb_tweet_tweetto']) ? '' : clean_slweg($_POST['fb_tweet_tweetto'])
	);

	if(empty($plugin_fb['data']['fb_name'])) {
		$plugin_fb['error']['fb_name'] = 1;
	}
	if(empty($plugin_fb['data']['values']['fb_tweet_tweetto'])) {
		$plugin_fb['error']['fb_tweet_tweetto'] = 1;
	}
		...
		if( empty($plugin_fb['error'] )) {
			...
			if( isset($_POST['save']) ) {
				...
				headerRedirect( fb_map_url( array('controller=tw_mention', 'edit='.$plugin_fb['data']['fb_id']), '') );
			}	
