...
if(isset($_POST['fb_name'])) {
  ...
  $plugin_fb['data']['values'] = array(
    'fb_tweet_button'	=> empty($_POST['fb_tweet_button']) ? 'medium' : clean_slweg($_POST['fb_tweet_button']),
    'fb_tweet_locale'		=> empty($_POST['fb_tweet_locale']) ? 'en' : clean_slweg($_POST['fb_tweet_locale']),
    'fb_tweet_followuser'		=> empty($_POST['fb_tweet_followuser']) ? '' : clean_slweg($_POST['fb_tweet_followuser']),
    'fb_tweet_showusername'			=> empty($_POST['fb_tweet_showusername']) ? 0 : intval($_POST['fb_tweet_showusername']),
    'fb_tweet_count'		=> empty($_POST['fb_tweet_count']) ? 'false' : clean_slweg($_POST['fb_tweet_count'])
	);

	if(empty($plugin_fb['data']['fb_name'])) {
		$plugin_fb['error']['fb_name'] = 1;
	}
	if(empty($plugin_fb['data']['values']['fb_tweet_followuser'])) {
		$plugin_fb['error']['fb_tweet_followuser'] = 1;
	}
		...
		if( empty($plugin_fb['error'] )) {
			...
			if( isset($_POST['save']) ) {
				...
				headerRedirect( fb_map_url( array('controller=twitterfollow', 'edit='.$plugin_fb['data']['fb_id']), '') );
			}	
...
if(empty($plugin_fb['data'])) {
    ...
    'fb_created'	=> '',
    'fb_changed'	=> date('Y-m-d H:i:s'),
  	'fb_cat'		=> 'twitterfollow',
  	'fb_name'		=> '',
  	'fb_status'		=> 0
	);
	$plugin_fb['data']['values'] = array(
    //'fb_twitter_followcode'	=> '',   //depreecated V 1.0.5
    'fb_tweet_button'	=> 'medium',
    'fb_tweet_locale'		=> 'en',
    'fb_tweet_followuser'		=> '',
    'fb_tweet_showusername'		=> 0,
    'fb_tweet_count'		=> 'false'
	);
}
?>