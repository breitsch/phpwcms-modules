...
jQuery.noConflict();
jQuery("#param_fb_tweet_hashtags").on("keyup blur", function() {PHPWCMS_MODULE.SP.validTwVal(this)});
jQuery("#param_fb_tweet_recom1,#param_fb_tweet_recom2").on("keyup blur", function(){PHPWCMS_MODULE.SP.validTwRecomm(this)});
jQuery("#param_fb_tweet_hashtags,#article_title").on("keyup blur", function(){PHPWCMS_MODULE.SP.requiredInput(this)});
...
<form action="<?php echo fb_map_url( array('controller=tw_hashtag', 'edit='.$plugin_fb['data']['fb_id']) ) ?>" name="frmtwitterbutton" method="post">
<input name="fb_id" type="hidden" value="<?php echo $plugin_fb['data']['fb_id'] ?>" />
<table width="100%" summary="" class="br_module_table">

  <tr>
		<td colspan="2">
      <div class="br_module_title"><?php echo $BLM['tit_tw_hashtag']; ?>
  ...
  <tr> 
	...
	<tr>
		<td>Hashtag&nbsp;#</td>
		<td><input type="text" name="fb_tweet_hashtags" id="param_fb_tweet_hashtags" class="<?php

		//error class
		if(!empty($plugin_fb['error']['fb_tweet_hashtags'])) echo ' errorInputText';

		?>" value="<?php echo $plugin_fb['data']['values']['fb_tweet_hashtags'] ?>" placeholder="hashtag" maxlength="255" /></td>
	</tr>
	<tr>
		<td></td>
		<td>Both within the button itself and in the text of the tweet status, the hashtag you specify will appear.</td>
	</tr>
  ...
  <tr><td colspan="2"><div class="br_module_spaceh10"></div></td></tr>
	...
	<tr>
		...
		<td><input type="radio" name="fb_tweet_domain" id="param_fb_tweet_domain_2" value="2" <?php is_checked('2', $plugin_fb['data']['values']['fb_tweet_domain']) ?> /><label for="param_fb_tweet_domain_2"><?php echo $BLM['fb_tw_url2'] ?></label></td>
	</tr>
	<tr>
		<td></td>
